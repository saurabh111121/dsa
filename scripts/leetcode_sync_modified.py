...
Modified version with improved error handling for premium/locked content.
"""
...
def check_solution_availability(graphql_url, headers, cookies, title_slug):
    """
    Check if a solution is available and not locked behind premium.
    ...
    Returns:
        tuple: (bool, dict) - (is_available, solution_data)
    """
    # Query for just checking solution availability
    solution_query = """
    query solutionAvailability($titleSlug: String!) {
    ...
    try:
        logger.debug(f"Checking solution availability for: {title_slug}")
        response = requests.post(
            graphql_url,
            headers=headers,
            cookies=cookies,
            json={'query': solution_query, 'variables': {'titleSlug': title_slug}},
            timeout=30
        )
        
        if response.status_code != 200:
            logger.error(f"Solution check failed with status code: {response.status_code}")
            return False, {}
        ...
        if solution:
            is_paid_only = solution.get('isPaidOnly', True)
            can_see_detail = solution.get('canSeeDetail', False)
            
            if is_paid_only and not can_see_detail:
                logger.info(f"Solution for {title_slug} is premium-only and cannot be accessed")
                return False, solution
            else:
                return True, solution
        else:
            logger.warning(f"No solution information available for {title_slug}")
            return False, {}
            
    except Exception as e:
        logger.error(f"Error checking solution availability: {str(e)}")
        return False, {}
...
def enhance_solutions(leetcode_dir, csrf_token, session_token):
    ...
    # Separate query for solutions to handle premium content better
    solution_query = """
    ...
    for problem_dir in all_problem_dirs:
        ...
        try:
            # First check if this problem's solution is available or premium-locked
            is_available, solution_info = check_solution_availability(graphql_url, headers, cookies, problem_slug)
            
            # Query LeetCode for basic problem details (without solution)
            response = requests.post(
                graphql_url,
                headers=headers,
                cookies=cookies,
            ...
            # Process solution content if available and not premium-locked
            solution_content = ""
            
            if is_available:
                # Fetch full solution content in a separate query
                try:
                    solution_response = requests.post(
                        graphql_url,
                        headers=headers,
                        cookies=cookies,
                        json={'query': solution_query, 'variables': {'titleSlug': problem_slug}},
                        timeout=30
                    )
                    
                    if solution_response.status_code == 200:
                        solution_data = solution_response.json()
                        solution = solution_data.get('data', {}).get('question', {}).get('solution', {})
                        
                        if solution and not solution.get('isPaidOnly', True) and solution.get('content'):
                            solution_html = solution.get('content', '')
                            solution_text = BeautifulSoup(solution_html, 'html.parser').get_text()
                            solution_content = f"\n\n## Official Solution\n{solution_text}"
                        else:
                            solution_content = "\n\n## Official Solution\n*This is a premium-only solution or no solution is available.*"
                    else:
                        logger.warning(f"Failed to fetch solution for {problem_slug}: {solution_response.status_code}")
                        solution_content = "\n\n## Official Solution\n*Solution could not be retrieved due to API error.*"
                except Exception as e:
                    logger.warning(f"Error fetching solution for {problem_slug}: {str(e)}")
                    solution_content = "\n\n## Official Solution\n*Solution could not be retrieved due to an error.*"
            else:
                # Solution is premium-locked or not available
...
"""
            ...
            errors += 1
            # Continue with next problem instead of stopping
            continue
...
def main():
    ...
    parser.add_argument('--skip-premium', action='store_true', help='Skip premium solutions to avoid errors')
